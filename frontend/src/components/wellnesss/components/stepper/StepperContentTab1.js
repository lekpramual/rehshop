...
function StepperContentTab1(props) {
  ...
  useEffect(() => {
    if (iscount) {
      ...
      setData((data) => ({
        ...data,
        sex: { value: `${isdata[0].sex}` },
        vstdate: { value: moment(isdata[0].vstdate).toDate() },
        pname: { value: isdata[0].pname },
        firstname: { value: isdata[0].fname },
        lastname: { value: isdata[0].lname },
        age: {
          value: calculate_age(
            moment(isdata[0].birthdate).format("YYYY-MM-DD")
          ),
        },
        tel: { value: `${isdata[0].phone}` },
        birthdate: {
          value: moment(isdata[0].birthdate).format("YYYY-MM-DD"),
        },
        address: { value: isdata[0].addr },
        hn: { value: isdata[0].hn },
        service: {
          value: `${isdata[0].come_type}`,
          label:
            isdata[0].come_type === 1 ? "มารับการตรวจเอง" : "ให้บริการออกตรวจ",
        },
        right: {
          value: `${isdata[0].pttype}`,
          label: `${isdata[0].pttype_name}`,
        },
        department: {
          value: isdata[0].dep_id,
          label: `${isdata[0].dep_name}`,
        },
      }));
    }
  }, [isdata, iscount, dispatch]);
